...
-- Requires zhparser to be compiled and installed in the PostgreSQL instance
CREATE EXTENSION IF NOT EXISTS zhparser;

-- Create Chinese text search configuration using zhparser
CREATE TEXT SEARCH CONFIGURATION chinese (PARSER = zhparser);

-- Map zhparser token types to simple dictionary
-- This is critical: without this mapping, to_tsvector will return empty
...
CREATE INDEX idx_doc_content_hash ON t_knowledge_document (file_content_hash, file_size);
...
CREATE TABLE t_knowledge_vector (
    ...
    tsv         tsvector  -- 全文检索向量，由 zhparser 分词自动生成
);
...
CREATE INDEX idx_kv_tsv ON t_knowledge_vector USING gin(tsv);  -- 全文检索 GIN 索引
...
-- Trigger function to automatically maintain tsv column
...
-- Trigger to update tsv on INSERT or UPDATE
