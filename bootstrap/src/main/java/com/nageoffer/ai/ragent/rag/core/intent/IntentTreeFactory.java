...
public class IntentTreeFactory {
    ...
    public static List<IntentNode> buildIntentTree() {
        ...
        IntentNode nl2sql = IntentNode.builder()
                .id("nl2sql")
                .name("数据库查询")
                .level(DOMAIN)
                .kind(IntentKind.MCP)
        ...
        IntentNode nl2sqlQuery = IntentNode.builder()
                .id("nl2sql-query")
                .name("数据库自然语言查询")
                .level(CATEGORY)
                .parentId(nl2sql.getId())
                .mcpToolId("nl2sql_query")
                .kind(IntentKind.MCP)
                .promptTemplate(MCP_NL2SQL_PROMPT_TEMPLATE)
                .description("通过自然语言查询数据库中的业务数据，如用户数量、会话记录、知识库文档统计、意图配置等")
                .examples(List.of(
                        "系统一共有多少用户？",
                        "最近7天创建的会话有多少？",
                        "各知识库有多少文档？",
        ...
        nl2sql.setChildren(List.of(nl2sqlQuery));
        roots.add(nl2sql);
    ...
    public static final String MCP_NL2SQL_PROMPT_TEMPLATE = """
            Hello，你是专业的企业智能数据助手。系统已通过自然语言数据库查询（NL2SQL）获取到了最新的【数据库查询结果】。
            你的任务是将这些结构化数据转化为**商业化、易读的自然语言**回复。

            【核心处理规则】
            1. **直接回答**：开门见山地回答用户问题，不要使用"根据数据/数据库显示"这类废话作为开头。
            2. **去技术化**：将字段名转换为业务术语，隐藏内部 ID 和主键。
            3. **格式化输出**：
               - **多条数据**：使用 Markdown 表格展示，表头用中文。
               - **单条/聚合数据**：使用分点或自然段落清晰表述。
               - **关键指标**：进行加粗处理。

            【异常与边界处理】
            1. **数据为空**：回答"当前未查询到相关数据记录"。
            2. **报错数据**：抱歉告知系统暂时无法获取数据。
            3. **SQL 执行错误**：如数据中包含错误信息，用自然语言描述问题。

            【禁止事项】
            - 严禁根据数据内容臆造不存在的结论。
            - 严禁透漏 SQL 语句、数据库结构或技术细节。
