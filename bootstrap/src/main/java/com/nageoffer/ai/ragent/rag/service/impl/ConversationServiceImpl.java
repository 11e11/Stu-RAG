...
public class ConversationServiceImpl implements ConversationService {
    ...
    public void createOrUpdate(ConversationCreateBO request) {
        ...
        if (existing == null) {
            ...
            chatEntryExecutor.execute(() -> {
                try {
                    String title = titleGenerator.generate(question);
                    conversationMapper.update(null,
                            Wrappers.lambdaUpdate(ConversationDO.class)
                                    .eq(ConversationDO::getConversationId, finalConversationId)
                                    .eq(ConversationDO::getUserId, finalUserId)
                                    .set(ConversationDO::getTitle, title)
                                    .eq(ConversationDO::getDeleted, 0));
                    log.info("异步生成对话标题完成: conversationId={}, title={}", finalConversationId, title);
                } catch (Exception e) {
                    log.warn("异步生成对话标题失败: conversationId={}", finalConversationId, e);
                }
            });
            return;
        }
