...
public class MultiChannelRetrievalEngine {
    ...
    public List<RetrievedChunk> retrieveKnowledgeChannels(List<SubQuestionIntent> subIntents, int topK) {
        ...
        // 保存检索到的 chunks 到 Trace extraData
        saveChunksToTrace(chunks);

        return chunks;
    }

    /**
     * 将检索到的 chunks 保存到 Trace 的 extraData 字段
     */
    private void saveChunksToTrace(List<RetrievedChunk> chunks) {
        ...
        try {
            String extraData = JSONUtil.toJsonStr(chunks);
            traceRecordService.updateNodeExtraData(traceId, nodeId, extraData);
            log.debug("保存检索 chunks 到 Trace extraData, traceId={}, nodeId={}, chunkCount={}", traceId, nodeId, chunks.size());
        } catch (Exception e) {
            log.warn("保存检索 chunks 到 Trace extraData 失败, traceId={}, nodeId={}", traceId, nodeId, e);
        }
    }
