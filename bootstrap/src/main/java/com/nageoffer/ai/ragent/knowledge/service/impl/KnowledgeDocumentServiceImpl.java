...
public class KnowledgeDocumentServiceImpl implements KnowledgeDocumentService {
    ...
    public KnowledgeDocumentVO upload(String kbId, KnowledgeDocumentUploadRequest requestParam, MultipartFile file) {
        ...
        // ========== 第一步：锁外读取字节 + 计算MD5（MultipartFile流只能读一次） ==========
        byte[] fileBytes = null;
        String fileContentHash = null;
        if (SourceType.FILE == sourceType && file != null) {
            try {
                fileBytes = file.getBytes();
                fileContentHash = computeMd5(fileBytes);
            } catch (Exception e) {
                log.error("读取文件或计算MD5失败", e);
                throw new ClientException("文件处理失败，请重试");
            }
        }

        // ========== 第二步：有MD5时走幂等锁保护路径 ==========
        if (StrUtil.isNotBlank(fileContentHash)) {
            String lockKey = "rag:document:idempotent:" + fileContentHash;
            RLock lock = redissonClient.getLock(lockKey);
            try {
                // 看门狗自动续期，避免大文件处理超时导致锁提前释放
                boolean locked = lock.tryLock(3, TimeUnit.SECONDS);
                if (!locked) {
                    throw new ClientException("文档处理中，请勿重复上传");
                }
                // ★ 临界区：查重 → 存储 → 入库，原子完成
                Long existCount = documentMapper.selectCount(
                        new LambdaQueryWrapper<KnowledgeDocumentDO>()
                                .eq(KnowledgeDocumentDO::getFileContentHash, fileContentHash)
                                .eq(KnowledgeDocumentDO::getKbId, kbId)
                                .eq(KnowledgeDocumentDO::getFileSize, (long) fileBytes.length)
                );
                if (existCount > 0) {
                    throw new ClientException("文档已存在，无需重复上传");
                }
                // 确认不重复后，才执行文件存储
                StoredFileDTO stored = fileStorageService.upload(
                        kbDO.getCollectionName(), fileBytes,
                        file.getOriginalFilename(), file.getContentType());
                ProcessModeConfig modeConfig = resolveProcessModeConfig(requestParam);

                KnowledgeDocumentDO documentDO = buildDocumentDO(
                        kbId, stored, modeConfig, sourceType, requestParam, fileContentHash);
                documentMapper.insert(documentDO);
                return BeanUtil.toBean(documentDO, KnowledgeDocumentVO.class);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new ClientException("获取文档锁失败，请稍后重试");
            } finally {
     ...
     * 构建文档实体对象
     */
    private KnowledgeDocumentDO buildDocumentDO(String kbId, StoredFileDTO stored,
                                               ProcessModeConfig modeConfig, SourceType sourceType,
                                               KnowledgeDocumentUploadRequest requestParam, String fileContentHash) {
