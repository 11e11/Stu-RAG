...
public class RagTraceAspect {
    ...
    public Object aroundNode(ProceedingJoinPoint joinPoint, RagTraceNode traceNode) throws Throwable {
        ...
        try {
            ...
            // 如果返回值是 List<RetrievedChunk> 类型，自动保存到 extraData
            log.debug("Trace node 完成, traceId={}, nodeId={}, resultType={}", traceId, nodeId, result == null ? "null" : result.getClass().getName());
            if (result instanceof List<?> list && !list.isEmpty()) {
                Object firstElement = list.get(0);
                log.debug("List 第一个元素类型: {}", firstElement == null ? "null" : firstElement.getClass().getName());
                if (firstElement instanceof RetrievedChunk) {
                    try {
                        String extraData = JSONUtil.toJsonStr(list);
                        log.info("保存 RetrievedChunk 到 extraData, traceId={}, nodeId={}, chunkCount={}", traceId, nodeId, list.size());
                        traceRecordService.updateNodeExtraData(traceId, nodeId, extraData);
                    } catch (Exception e) {
                        log.warn("保存 RetrievedChunk 到 extraData 失败, traceId={}, nodeId={}", traceId, nodeId, e);
                    }
                }
            }
